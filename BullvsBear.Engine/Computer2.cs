...
namespace Acquire.Engine
{
    ...
    public class Computer2 : IComputer
    {
        ...
        private TileOption[] CategorizeTiles(Square[] tiles, out bool hasNewCorp, out bool hasMerger, out bool hasCorpExpand)
        {
            ...
            for (int i = 0; i < tiles.Length; i++)
            {
                ...
                else if (types[i].Merger.Count > 1)
                {
                    ...
                    {
                        if (Corporations[(int)corp].Size > BullvsBearConstants.NonMergableCorp) corpsAboveMax++;
                    }
        ...
        public Square SelectSquare(Square[] tiles)
        {
            ...
            if (hasMerger)
            {
                ...
                if (Corporations[(int)largestCorp].Shares == 0 
                    || Player.Cash < ((Corporations[(int)largestCorp].Shares > BullvsBearConstants.MaxSharePurchase)? (Corporations[(int)largestCorp].Price * BullvsBearConstants.MaxSharePurchase) : (Corporations[(int)largestCorp].Price * Corporations[(int)largestCorp].Shares))
                    || lastSkippedMerger)
                {
        ...
        public CorpNames[] BuyShares(List<CorpNames>[] corps)
        {
            ...
            int[] remaining = new int[BullvsBearConstants.CorpCount]; // track how many of each corp remain
            int cash;
            int index;
            ...
            else
            {
                for (int tries = 0; tries < corps.Length; tries++)
                {
                    ...
                    foreach (CorpStats corp in Corporations)
                    {
                        if (corp.Size > BullvsBearConstants.NonMergableCorp)
                        {
                            // check that we have a reasonable chance of being the majority
                            if (index >= 0
                                && remaining[(int)corp.Name] > 0
                                && Player.Shares(corp.Name) < ((BullvsBearConstants.MaxShares - corp.Shares) / NumberPlayers)
                                && cash >= corp.Price)
                            {
                    ...
                    for (int dim0 = BullvsBearConstants.BoardHeight/4; dim0 < (BullvsBearConstants.BoardHeight*3)/4 && index >= 0; dim0++)
                    {
                        for (int dim1 = BullvsBearConstants.BoardWidth/4; dim1 < (BullvsBearConstants.BoardWidth*3)/4 && index >= 0; dim1++)
                        {
                    ...
                    for (int size = 2; size <= BullvsBearConstants.NonMergableCorp && index >= 0; size++)
                    {
                        foreach (CorpStats corp in Corporations)
                        {
                            if (index >= 0
