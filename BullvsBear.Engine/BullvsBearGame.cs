...
{
    public enum CorpNames { CocaCola=0, Samsung=1, Kakao=2, BaskinRobbins=3, Naver=4, FedEx=5, Apple= 6, NA=7, UnClaimed=8 };
    public enum BullvsBearGameStates { GameSetup, PlaceTile, SellShares, ChooseCorp, BuyShares, ChooseParentCorp, TradeShares, NextTurn, ExitGame, Done };
    ...
    public class CorpStats
    {
        public CorpStats(CorpNames name, int size)
        {
            Name = name;
            Size = size;
            Shares = BullvsBearConstants.MaxShares; // default starting value
        }
        ...
        public int Price
        {
            get
            {
                ...
                if (Name.Equals(CorpNames.CocaCola) || Name.Equals(CorpNames.Samsung))
                {
                ...
                else if (Name.Equals(CorpNames.Kakao) || Name.Equals(CorpNames.BaskinRobbins) || Name.Equals(CorpNames.Naver))
                {
                ...
                else if (Name.Equals(CorpNames.Apple) || Name.Equals(CorpNames.FedEx))
                {
    ...
    public class PlayerStats
    {
        ...
        public PlayerStats(string name, bool computer, int id)
        {
            ...
            Cash = BullvsBearConstants.InitialCash;
            Name = name;
            IsComputer = computer;
            tiles = new List<Square>();
            ID = id;

            shares = new int[BullvsBearConstants.CorpCount];
            for(int c=0; c<BullvsBearConstants.CorpCount; c++) shares[c] = 0;
        }
        ...
        {
            if (tiles.Count > BullvsBearConstants.MaxTiles) throw new ArgumentException("Added to many tiles to '" + Name + "'s tile collection");
            tiles.Add(tile);
            return true;
        }
    ...
    class TileCollection
    {
        ...
        public TileCollection(int seed = 0)
        {
            ...
            tiles = new Square[BullvsBearConstants.BoardWidth * BullvsBearConstants.BoardHeight];
            for(int Dim0=0; Dim0<BullvsBearConstants.BoardHeight; Dim0++)
            {
                for (int Dim1=0; Dim1<BullvsBearConstants.BoardWidth; Dim1++)
                {
                    // initialize the set of tiles
                    tiles[Dim1 + (Dim0*BullvsBearConstants.BoardHeight)].Dim0 = Dim0;
                    tiles[Dim1 + (Dim0*BullvsBearConstants.BoardHeight)].Dim1 = Dim1;
                }
            }
            ...
            for (int i=0; i<500; i++)
            {
                int i1 = rand.Next() % (BullvsBearConstants.BoardHeight*BullvsBearConstants.BoardWidth);
                int i2 = rand.Next() % (BullvsBearConstants.BoardHeight*BullvsBearConstants.BoardWidth);
    ...
    class Board
    {
        ...
        public Board()
        {
            board = new CorpNames[BullvsBearConstants.BoardHeight, BullvsBearConstants.BoardWidth];
            for (int Dim0 = 0; Dim0 < BullvsBearConstants.BoardHeight; Dim0++)
            {
                for (int Dim1 = 0; Dim1 < BullvsBearConstants.BoardWidth; Dim1++)
                {
        ...
        public BoardStates TryPlace(Square tile, CorpStats[] corps, out List<CorpNames> merger, out CorpNames expandedCorp)
        {
            ...
            //    BoardStates.InvalidAlways - Merge corp, but 2 or more are greater than BullvsBearConstants.NonMergableCorp
            ...
            if (merger.Count > 1)
            {
                ...
                {
                    if (corps[(int)corp].Size > BullvsBearConstants.NonMergableCorp) corpsAboveMax++;
                }
    ...
    public static class BullvsBearConstants
    {
    ...
    public class BullvsBearGame
    {
        private CorpStats[] corporations;
        private List<PlayerStats> players;
        private BullvsBearGameStates state;
        ...
        public BullvsBearGame(int seed = 0)
        {
            // initialize content
            corporations = new CorpStats[BullvsBearConstants.CorpCount];
            for (int c = 0; c < BullvsBearConstants.CorpCount; c++) corporations[c] = new CorpStats((CorpNames)c, 0);

            players = new List<PlayerStats>();
            state = BullvsBearGameStates.GameSetup;
            currentPlayer = -1;
        ...
        public BullvsBearGameStates State
        {
            get
            {
                return state;
        ...
        public List<CorpNames> ParentCorporations
        {
            get
            {
                if (BullvsBearGameStates.ChooseParentCorp != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
                return parentCorps;
            }
        }
        ...
        public CorpNames ParentCorporation
        {
            get
            {
                if (BullvsBearGameStates.TradeShares != state 
                    && BullvsBearGameStates.SellShares != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
        ...
        public List<CorpNames> AvailableCorporations
        {
            get
            {
                if (BullvsBearGameStates.ChooseCorp != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
        ...
        public TradeTransaction SharesToTrade
        {
            get
            {
                ...
                if (BullvsBearGameStates.TradeShares != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
        ...
        public SaleTransaction SharesToSell
        {
            get
            {
                if (BullvsBearGameStates.SellShares != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
        ...
        public List<CorpNames>[] SharesToBuy
        {
            get
            {
                ...
                if (BullvsBearGameStates.BuyShares != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);

                // init
                corps = new List<CorpNames>[BullvsBearConstants.MaxSharePurchase];
                for (int i = 0; i < BullvsBearConstants.MaxSharePurchase; i++) corps[i] = new List<CorpNames>();

                // find all Corps with MaxSharePurchase+/.../2+/1+ shares and Size > 0
                foreach (CorpStats corp in corporations)
                {
                    for (int i = BullvsBearConstants.MaxSharePurchase; i > 0; i--)
        ...
        ///////////////////////////
        // BullvsBearGameStates.GameSetup
        public int AddPlayer(string name, bool computer)
        {
            PlayerStats player;

            if (BullvsBearGameStates.GameSetup != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);

            if (players.Count >= BullvsBearConstants.MaxPlayers) throw new ArgumentException("Cannot exceed " + BullvsBearConstants.MaxPlayers + " players");

            player = new PlayerStats(name, computer, gid++);
            for (int i = 0; i < BullvsBearConstants.MaxTiles; i++) player.AddTile(tiles.Next());
            players.Add(player);
        ...
        public bool StartGame()
        {
            ...
            state = BullvsBearGameStates.NextTurn;
            AdvancePlayer();
        ...
        ///////////////////////////
        // BullvsBearGameStates.ExitGame
        public bool EndGame()
        {
            if (BullvsBearGameStates.ExitGame != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);

            state = BullvsBearGameStates.Done;
        ...
        private void AdvancePlayer()
        {
            ...
            if (BullvsBearGameStates.ExitGame != state && BullvsBearGameStates.NextTurn != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
            ...
            state = BullvsBearGameStates.PlaceTile;
        }

        ///////////////////////////
        // BullvsBearGameStates.PlaceTile
        public bool ShuffleTiles()
        {
            if (BullvsBearGameStates.PlaceTile != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
            ...
            state = BullvsBearGameStates.BuyShares;

            return true;
        }

        ///////////////////////////
        // BullvsBearGameStates.NextTurn
        public bool EndTurn()
        {
            int totalShares;
            int totalSize;

            if (BullvsBearGameStates.NextTurn != state) throw new ArgumentException("BullvsBearGame not in the right stete: " + state);
            ...
            foreach (CorpStats corp in corporations)
            {
                if (corp.Shares < 0 || corp.Shares > BullvsBearConstants.MaxShares) throw new ArgumentException("Corporation shares is out of range '" + corp.Shares + "'");
            ...
            if (totalShares != (BullvsBearConstants.MaxShares * BullvsBearConstants.CorpCount)) throw new ArgumentException("Totalshares is inconsistent '" + totalShares + "' should be '" + (BullvsBearConstants.MaxShares * BullvsBearConstants.CorpCount) + "'");
            ...
            {
                // change states
                state = BullvsBearGameStates.ExitGame;
            }
            else
            {
                // move ahead
        ...
        private bool GameOver()
        {
            ...
            foreach (CorpStats corp in corporations)
            {
                if (corp.Size >= BullvsBearConstants.LargestCorporation) return true;
            }
        ...
        public bool StartTurn(Square tile)
        {
            ...
            switch (tileState)
            {
                ...
                case BoardStates.MergeCorp:
                    ...
                    state = BullvsBearGameStates.ChooseParentCorp;
                    return true;
                case BoardStates.NewCorp:
                    ...
                    state = BullvsBearGameStates.ChooseCorp;
                    return true;
            }
        ...
        ///////////////////////////
        // BullvsBearGameStates.ChooseParentCorp
        public bool ChooseParentCorp(CorpNames parentCorp)
        {
            ...
            // change state
            state = BullvsBearGameStates.TradeShares;

            return true;
        }

        ///////////////////////////
        // BullvsBearGameStates.TradeShares
        public bool TradeShares(int count)
        {
            ...
            if (tradeShares.Count == 0) state = BullvsBearGameStates.SellShares;

            return true;
        }

        ///////////////////////////
        // BullvsBearGameStates.SellShares
        ...
        ///////////////////////////
        // BullvsBearGameStates.ChooseCorp
        ...
        ///////////////////////////
        // BullvsBearGameStates.BuyShares
        public bool BuyShares(CorpNames[] corps)
        {
            ...
            if (corps.Length > BullvsBearConstants.MaxSharePurchase) throw new ArgumentException("Must pass in " + BullvsBearConstants.MaxSharePurchase + " shares to buy");
